...
class Meituan:
    ...
    def __init__(self,data):
        self.pushPlus = data["pushPlus"]
        self.pushPlusToken = self.pushPlus["token"]
        self.pushPlusUse = self.pushPlus["use"]
        self.pushPlusUrl = "https://www.pushplus.plus"

        self.serverChan = data["serverChan"]
        self.serverChanToken = self.serverChan["token"]
        self.serverChanUse = self.serverChan["use"]
        self.serverChanUrl = "https://sct.ftqq.com"
    ...
    def pushPlus(self):
        global webhook
        pushPlusToken = self.pushPlusToken
        if not os.path.exists(str(cwd)+r"/output.txt"):
            print("output.txt文件异常,推送退出！🙌")
            return -1
        f = open(str(cwd)+r"/output.txt", mode='r',encoding="UTF-8")
        message = str(f.read())
    ...
    #定义server 酱的消息推送函数
    def serverjiang():
        serverkey = self.serverChanToken
        if not os.path.exists(str(cwd)+r"/output.txt"):
            print("output.txt文件异常,推送退出！🙌")
            return -1
        f = open(str(cwd)+r"/output.txt", mode='r',encoding="UTF-8")
    ...
    def getVar(self):
        wm_latitude  = self.latitude
        wm_longitude = self.longitude
    ...
    def myredbean(self):
        ...
        datas = "parActivityId="+parActivityId+"&wm_latitude="+wm_latitude+"&wm_longitude="+wm_longitude+"&token="+str(token)+"&userPortraitId="+str(portraitId)
    ...
    def getbatchId(self):
        token = self.token
        wm_latitude = self.getVar()[0]
        wm_longitude = self.getVar()[1]
        print("**开始执行获取batchId脚本:**\n")
        datas = "parActivityId="+parActivityId+"&wm_ctype="+wm_ctype+"&wm_latitude="+wm_latitude+"&wm_longitude="+wm_longitude+"&token="+token
    ...
    #def 限时抢红包函数
    def drawlottery(self,batchId,token,propIdforuse):
        wm_latitude = self.getVar()[0]
        wm_longitude = self.getVar()[1]
        print("**开始执行限时抢天天神券脚本🧧:**\n")
    ...
    def acceptRed(self,batchId,token):
        wm_latitude = self.getVar()[0]
        wm_longitude = self.getVar()[1]
        print("**开始执行发放天天神券🧧到红包库脚本:**\n")
        datas = "parActivityId="+parActivityId+"&wm_latitude="+wm_latitude+"&wm_longitude="+wm_longitude+"&token="+token+"&batchId="+batchId
    ...
    def redtobean(self,batchId,token):
        wm_latitude = self.getVar()[0]
        wm_longitude = self.getVar()[1]
    ...
    def sendTaskRedBean(self):
        ...
        datas = "parActivityId="+parActivityId+"&wm_latitude="+wm_latitude+"&wm_longitude="+wm_longitude+"&token="+token+"&portraitId="+str(portraitId)
    ...
    def doAction(self):
        ...
        datas = "parActivityId="+parActivityId+"&wm_latitude="+wm_latitude+"&wm_longitude="+wm_longitude+"&token="+token+"&action=SiginInGetProp"
    ...
    def querymyProps(self):
        ...
        datas = "parActivityId="+parActivityId+"&wm_latitude="+wm_latitude+"&wm_longitude="+wm_longitude+"&token="+token
    ...
    def exchange(self):
        ...
        propId = self.getVar()[3]
        exchangeCoinNumber = self.getVar()[4]
        print("**开始执行每日豆子兑换必中符脚本**:\n")
        while(1):
            datas = "wm_actual_longitude="+wm_actual_longitude+"&wm_actual_latitude="+wm_actual_latitude+"&exchangeRuleId=&propId="+str(propId)+"&exchangeCoinNumber="+str(exchangeCoinNumber)+"&parActivityId="+parActivityId+"&wm_ctype="+wm_ctype+"&wm_latitude="+wm_latitude+"&wm_longitude="+wm_longitude+"&token="+token
    ...
    def myRedBeanRecords(self):
        ...
        datas = "parActivityId="+parActivityId+"&wm_latitude="+wm_latitude+"&wm_longitude="+wm_longitude+"&token="+str(token)+"&userPortraitId="+str(portraitId)+"&pageNum=1"
    ...
    def queryredpool(self):
        ...
        wm_longitude = self.getVar()[1]
        print("**开始执行查询红包池详情脚本:**\n")
        datas = "parActivityId="+parActivityId+"&wm_latitude="+wm_latitude+"&wm_longitude="+wm_longitude+"&token="+str(token)+"&wm_ctype="+wm_ctype
...
def main():
    ...
    # Storage config file data
    config_data = None
    try:
        with open(file_path+'/'+config_file) as f:
    ...
    meituan = Meituan(config_data)
    token = meituan.token
    meituan.getpropId_Coinnumber()
    sys.stdout = Logger(str(cwd)+r'/output.txt')
    meituan.signForBeans()
    
    meituan.queryredpool()
    batchId = meituan.getbatchId()
    ##先去保持每天签到 以获得必中符或者豆子
    meituan.doAction()
    meituan.exchange()   
    meituan.querymyProps()
    ...
    if n_time < d_time7  :
        propIdforuse =1      
    meituan.drawlottery(batchId,token,propIdforuse)

    if(int(showPriceNumber)<500):
        meituan.redtobean(batchId,token)
    else:
        meituan.acceptRed(batchId,token)
    meituan.querymyreward()
    meituan.sendTaskRedBean()
    meituan.querymyProps()
    meituan.myRedBeanRecords()
    sys.stdout = temp
    if meituan.pushPlusUse == True:
        meituan.pushPlus()
    else:
        print("您已默认关闭pushPlus推送!若需开启,请将pushPlusToken 填入本脚本目录下的pushPlusToken.txt文本中!\n")

    if meituan.serverChanUse == True:
        if n_time>d_time0:
            meituan.serverjiang()
        else:
